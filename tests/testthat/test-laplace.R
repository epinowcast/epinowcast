...
test_that("enw_laplace_marginal_data builds a valid data list for an RE expectation", {
  expn <- enw_expectation(~ 1 + (1 | day_of_week), data = pobs)
  dl <- enw_laplace_marginal_data(pobs, expectation = expn)
  expect_type(dl, "list")
...
test_that("enw_laplace_marginal_data builds a gp() Hilbert-space basis block", {
...
test_that("enw_laplace_marginal_data observed cells respect the triangle boundary", {
...
test_that("enw_laplace_marginal_data maps the observation family", {
  dl_nb <- enw_laplace_marginal_data(pobs, obs = enw_obs("negbin", data = pobs))
  dl_po <- enw_laplace_marginal_data(pobs, obs = enw_obs("poisson", data = pobs))
...
test_that("the NB1 (negbin1d) family is rejected", {
  expect_error(
    enw_laplace_marginal_data(pobs, obs = enw_obs("negbin1d", data = pobs)),
    "negbin"
  )
})
...
test_that("a non-parametric reference model is rejected", {
  ...
  expect_error(
    enw_laplace_marginal_data(pobs, reference = ref),
    "non-parametric"
  )
})
...
test_that("a missing-reference model is rejected", {
  ...
  expect_error(
    enw_laplace_marginal_data(pobs, missing = miss),
    "missing-reference"
  )
})

test_that("enw_laplace_marginal_inits returns finite starting values", {
  dl <- enw_laplace_marginal_data(
