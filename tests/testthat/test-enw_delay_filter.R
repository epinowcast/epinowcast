...
test_that("enw_delay_filter() filters correctly for a week timestep", {
  obs <- data.frame(
    report_date = as.Date("2021-01-01") + 0:20,
    reference_date = as.Date("2021-01-01")
  )
  filtered_obs <- enw_delay_filter(obs, max_delay = 2, timestep = "week")
...
test_that("enw_delay_filter() filters correctly for a 5 days timestep", {
  obs <- data.frame(
    report_date = as.Date("2021-01-01") + 0:20,
    reference_date = as.Date("2021-01-01")
  )
  filtered_obs <- enw_delay_filter(obs, max_delay = 2, timestep = 5)
  expect_true(all(filtered_obs$report_date <= as.Date("2021-01-01") + 10))
  expect_true(all(filtered_obs$report_date >= as.Date("2021-01-01")))
})

test_that("enw_delay_filter() throws a warning for week timestep when empirical delay is less", {
  obs <- data.frame(
    report_date = as.Date("2021-01-01") + 0:6,
    reference_date = as.Date("2021-01-01")
  )
  expect_warning(enw_delay_filter(obs, max_delay = 10, timestep = "week"))
})
