...
vignette](https://package.epinowcast.org/articles/model.md). For
...
| **Delay-only model** | Estimate the reporting delay given known totals, without a latent process | [Delay estimation vignette](https://package.epinowcast.org/articles/delay-estimation.md) |
...
|----|----|----|----|
| Daily | High resolution surveillance | `timestep = "day"` (default) | [Getting started vignette](https://package.epinowcast.org/articles/epinowcast.md) |
| Weekly | Reduced computational cost | `timestep = "week"` | [Temporal aggregation guide](https://package.epinowcast.org/articles/temporal-aggregation.md) |
| Daily process, weekly reporting | Daily inference from once-a-week reporting | `timestep = "day"` with weekly observation scaffold | [Temporal aggregation guide](https://package.epinowcast.org/articles/temporal-aggregation.md) |
...
vignette](https://package.epinowcast.org/articles/epinowcast.md) uses
daily data. For weekly data, weekly reporting on a daily process, and a
side-by-side comparison with the daily benchmark see the [temporal
aggregation
...
|----|----|----|----|
| Age groups | Age-stratified surveillance | `by = c("age_group")` | [Germany age-stratified vignette](https://package.epinowcast.org/articles/germany-age-stratified-nowcasting.md) |
| Geographic regions | Regional nowcasts | `by = c("region")` | Set `by` in [`enw_preprocess_data()`](https://package.epinowcast.org/reference/enw_preprocess_data.md) |
...
study](https://package.epinowcast.org/articles/germany-age-stratified-nowcasting.md)
...
| Delay-only model | Estimate the delay given known totals, without a latent process | `enw_obs(delay_only = TRUE)` | [Delay estimation vignette](https://package.epinowcast.org/articles/delay-estimation.md) |

**Available distributions:** See the [distributions
vignette](https://package.epinowcast.org/articles/distributions.md) for
details on the range of supported distributions.
...
| Custom structural patterns | Complex reporting schedules | [`enw_structural_reporting_metadata()`](https://package.epinowcast.org/reference/enw_structural_reporting_metadata.md) with custom data | Flexible aggregation |
...
| Sparse design | Memory efficient | `sparse = TRUE` in [`enw_formula()`](https://package.epinowcast.org/reference/enw_formula.md) | Large sparse matrices |
...
| Missing observations | Control which observations are used in the likelihood | Forecasting, excluding outliers, handling NAs, testing parameter recovery | Set `.observed = FALSE` and use `observation_indicator` in [`enw_obs()`](https://package.epinowcast.org/reference/enw_obs.md) |

**Common missing observation workflows:**

| Use case | How to implement |
|----|----|
| Forecasting | [`enw_extend_date()`](https://package.epinowcast.org/reference/enw_extend_date.md) to add future dates marked as unobserved |
| Exclude outliers | Manually set `.observed = FALSE` for outlier observations |
| Handle NAs | [`enw_flag_observed_observations()`](https://package.epinowcast.org/reference/enw_flag_observed_observations.md) marks NA values as unobserved |
...
- NA handling example: See
  [`?enw_flag_observed_observations`](https://package.epinowcast.org/reference/enw_flag_observed_observations.md)
  and [`?enw_obs`](https://package.epinowcast.org/reference/enw_obs.md)
  for examples
...
|----|----|----|----|
| Posterior predictive checks | Visual fit assessment | `enw_fit_opts(pp = TRUE)` | [Model vignette](https://package.epinowcast.org/articles/model.md) |
| LOO-CV | Model comparison | `enw_fit_opts(output_loglik = TRUE)` + `loo` package | See [`?loo::loo`](https://mc-stan.org/loo/reference/loo.html) |
| Scoring rules | Probabilistic forecast evaluation | `scoringutils` package | [Germany hierarchical vignette](https://package.epinowcast.org/articles/germany-age-stratified-nowcasting.md) |
| Convergence diagnostics | MCMC quality checks | Check `$fit$summary()` | [Stan help vignette](https://package.epinowcast.org/articles/stan-help.md) |
...
|----|----|----|
| Convert from line list | [`enw_linelist_to_incidence()`](https://package.epinowcast.org/reference/enw_linelist_to_incidence.md) | Individual → aggregate data |
| Complete date sequences | [`enw_complete_dates()`](https://package.epinowcast.org/reference/enw_complete_dates.md) | Fill missing dates |
| Filter by dates | [`enw_filter_report_dates()`](https://package.epinowcast.org/reference/enw_filter_report_dates.md), [`enw_filter_reference_dates()`](https://package.epinowcast.org/reference/enw_filter_reference_dates.md) | Subset data |
| Add metadata | [`enw_add_metaobs_features()`](https://package.epinowcast.org/reference/enw_add_metaobs_features.md) | Day of week, holidays, etc. |
| Change timesteps | [`enw_aggregate_cumulative()`](https://package.epinowcast.org/reference/enw_aggregate_cumulative.md) | Daily → weekly, etc. |
| Main preprocessing | [`enw_preprocess_data()`](https://package.epinowcast.org/reference/enw_preprocess_data.md) | All-in-one wrapper |

**Where to see it:** The [getting started
vignette](https://package.epinowcast.org/articles/epinowcast.md)
