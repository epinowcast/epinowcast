...
real delay_group_lpmf(array[] int groups, int start, int end, array[] int obs,
  ...
  // indexing by group
  // need to know which report date data is linked (i.e we need a new vector look-up)
  // sum by this look-up to get cases by report date and group (can also make this a matrix)
