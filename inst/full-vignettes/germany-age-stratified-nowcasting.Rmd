...
plot(nowcast, type = "posterior", col = age_group, fill = age_group) +
...
To speed up model fitting we make use of posterior information from the previous model (with slight inflation) for some parameters. Note that this is not a truely Bayesian approach and in some situtations may cause issues. 

```{r}
params <- c("logmean_int", "logsd_int", "sqrt_phi")
posteriors <- nowcast$fit[[1]]$summary(params)
posteriors <- setDT(posteriors)[, sd := sd * 2]

priors <- enw_priors()
priors[!(variable %in% params)]
...
Nowcast performance looks visually improved but there is notable variation across age groups with the 35-59 year old nowcast appearing quite poor (and as a result the aggregate nowcast also not showing great performance).
...
As we have only nowcast a single date this evaluation is anything but complete however we can qualitatively assess the models for this report date. 

We first list all models,

```{r}
nowcasts <- list(
  baseline = nowcast, dow = dow_nowcast, age = age_nowcast, week = week_nowcast
)
```
...
More rigorously, we can evaluate the nowcasts using proper scoring rules from the `scoringutils` package including the weighted interval score, and relative pairwise performance.
