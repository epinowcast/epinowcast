...
Construct a parameric lognormal intercept only model for the date of reference using the metadata produced by `enw_preprocess_data()`. Note that `epinowcast` uses a sparse design matrix for parametric delay distributions to reduce runtimes so the design matrix shows only unique rows with `index` containing the mapping to the full design matrix.
...
nowcast <- epinowcast(pobs,
  ...
  fit = enw_fit_opts(
    sampler = function(init, ...){
      return(data.table::data.table(init = list(init)))
    },
...
```{r pp, fig.width = 16, fig.height = 16, message = FALSE, eval = FALSE}
