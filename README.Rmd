...
Below we first filter for a snapshot of retrospective data available 40 days before the 1st of October that contains 40 days of data and then produce the nowcast target based on the latest available hospitalisations by date of positive test. This will allow us to visualise how our nowcast made at the time does against what was ultimately reported.
...
This data is already in a format that can be used with `epinowcast` as there is a reference date (`reference_date`, the date of the observatio in this example the date of a positive test), a report date (`report_date`, the date of report for a given set of observations by reference date), and a count (`confirm`, the total (i.e. cumulative) number of hospitalisations by reference date and report date). The package also provides a range of tools to convert from line list, incidence, or other common formats (see [Data converters](https://package.epinowcast.org/dev/reference/index.html#data-converters)).
