...
[`rw()`](https://package.epinowcast.org/reference/rw.md)) where these
features can be used in the design matrix with the appropriate formula.
Supports grouping via the optional `.group` column. Note that cumulative
membership is indexed to start with zero (i.e. the first observation is
assigned a cumulative membership of zero).
