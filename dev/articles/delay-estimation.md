...
The delay-only likelihood is a multinomial conditional on the known
totals, so no observation `family` is needed. There is no expectation
module to configure: because the known totals override the expected
observations,
