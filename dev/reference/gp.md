...
with [`arima()`](https://package.epinowcast.org/dev/reference/arima.md),
arguments are not evaluated; they are passed by name for use in model
construction.

Like [`arima()`](https://package.epinowcast.org/dev/reference/arima.md)
and [`rw()`](https://package.epinowcast.org/dev/reference/rw.md), a
`gp()` term works on every module that takes a formula, each with its
own prior prefix:
...
- d:
  ...
  the per-group realisation is integrated (cumulative-summed) `d` times
