# ARMA alias for [`arima()`](https://package.epinowcast.org/dev/reference/arima.md)

Thin wrapper around
[`arima()`](https://package.epinowcast.org/dev/reference/arima.md) that
fixes `d = 0`. Equivalent to `arima(time, by, p = p, d = 0, q = q)`. For
an integrated (random-walk) series use
[`rw()`](https://package.epinowcast.org/dev/reference/rw.md) or
