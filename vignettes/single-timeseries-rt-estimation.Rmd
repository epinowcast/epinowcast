...
#> Error in expectation_module(data = rt_germany_pobs): object 'gt_pmf' not found
```
...
#> Error in expectation_module(data = retro_germany_pobs): object 'gt_pmf' not found
```
...
#> Error in eval(expr, envir, enclos): object 'germany_nowcast' not found
```
...
#> Error in eval(expr, envir, enclos): object 'nowcast_cdf' not found
```
...
#> Error in `[.data.table`(enw_latest_data(germany_hosp)[, `:=`(date, reference_date)], : exp_cases is not found in calling scope and it is not a column name either. When the first argument inside DT[...] is a single symbol (e.g. DT[var]), data.table looks for var in calling scope.
...
ggplot(exp_cases) +
  ...
  scale_fill_brewer(palette = "Dark2", aesthetics = c("color", "fill")) +
  theme(legend.position = "bottom")
#> Error in eval(expr, envir, enclos): object 'exp_cases' not found
```
