...
# and normalise it to account for right truncation
  (\(x) x / sum(x))()
#> Error in simulate_double_censored_pmf(max = 15, shape = gamma_shape, scale = gamma_scale, : could not find function "simulate_double_censored_pmf"

plot(gt_pmf)
#> Error in eval(expr, envir, enclos): object 'gt_pmf' not found
```
...
# and scale it by the infection to hospitalisation ration (2%)
  (\(x) x * 0.02)()
#> Error in simulate_double_censored_pmf(max = 15, meanlog = meanlog, sdlog = sdlog, : could not find function "simulate_double_censored_pmf"

plot(reporting_pmf)
#> Error in eval(expr, envir, enclos): object 'reporting_pmf' not found
```
...
#> 1: <data.table[30x4]>   62        62         1        30 2021-07-01
```
...
#> 1: <data.table[120x10]> <data.table[30x4]>   62        62         1        30
#>      max_date
#> 1: 2021-07-30
```
