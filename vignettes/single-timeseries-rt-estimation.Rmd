...
::: {.alert .alert-secondary}
  ...
  - [`EpiEstim`](https://cran.r-project.org/web/packages/EpiEstim/index.html): This is a more mature package for estimating the effective reproduction number. It exploits a mathematically relationship to fit the renewal equation with uncertainty very quickly but is not currently able to handle reporting delays.


  ### Further work {-}

    - Use a more flexible reporting delay distribution, potentially on ne that varies over time.
    - Use a generation time distribution that incorporates uncertainty and that also potentially changes over time.
