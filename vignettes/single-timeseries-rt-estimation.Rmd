...
*This vignette borrows heaviliy from this [nowcasting example](https://github.com/epiforecasts/nowcasting.example) by Sebastian Funk and Sam Abbott.*
...
As for the synthetic example we need to make a retrospective dataset and get the latest observations for the same time period as this will allows us to compare real-time estimates with what was ultimately retrospectively observed. Again as in the synthetic example we use 40 days of data with a maximum delay of 40 days.
...
```{r}
# Preprocess observations
germany_pobs <- enw_preprocess_data(retro_germany, max_delay = 30)
germany_pobs
```
...
## Summarise the nowcast

```{r check-synth-nowcast}
germany_nowcast |>
