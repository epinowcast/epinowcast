...
Each reference date has a known total, and its cells are a multinomial draw of that total across delays using the delay PMF.
We use a modest number of reference dates and a modest total per date so the posterior has visible uncertainty rather than collapsing to a point.
...
Each coloured line is one posterior draw, so the spread of lines is the posterior uncertainty in the delay distribution.
The draws bracket the simulated truth (dashed) for both models, so both recover the delay; the delay-only posterior (the tighter band) does not carry latent-process uncertainty.
...
The posterior delay parameters can be passed as priors to a subsequent full model through the `priors` argument of `epinowcast()`, either as a named list of `distspec` distributions (e.g. `list(refp_mean_int = distspec::Normal(mean = 1.5, sd = 0.2))`) or as the posterior summary from `summary(delay_fit, type = "fit")`, so the full model starts from a data-driven delay rather than the package defaults.
See `?enw_replace_priors` for the supported formats.
