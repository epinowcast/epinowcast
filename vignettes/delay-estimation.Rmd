...
The posterior delay parameters can be passed as priors to a subsequent full model through the `priors` argument of `epinowcast()`, either as a named list of `distspec` distributions (e.g. `list(refp_mean_int = distspec::Normal(mean = 1.5, sd = 0.2))`) or as the posterior summary from `summary(fit, type = "fit")`, so the full model starts from a data-driven delay rather than the package defaults.
