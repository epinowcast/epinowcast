...
#> Realised attack size: 4000 of 4000 susceptibles
```
...
We work from the posterior draws rather than a single summary, so the
comparison reflects the full posterior.


``` r
attack_draws <- function(nowcast, label) {
  draws <- posterior::as_draws_matrix(nowcast$fit[[1]]$draws("exp_llatent"))
  data.table(model = label, attack = rowSums(exp(draws)))
}

attack <- rbind(
  attack_draws(nowcast_adjusted, "adjusted"),
  attack_draws(nowcast_unadjusted, "unadjusted")
)

attack_summary <- attack[, .(
  median = median(attack),
  q5 = quantile(attack, 0.05),
  q95 = quantile(attack, 0.95)
), by = model]
knitr::kable(attack_summary, digits = 0, caption = paste(
...
Table: Recovered cumulative attack size. True value: 4000

|model      | median|   q5|  q95|
|:----------|------:|----:|----:|
|adjusted   |   4000| 4000| 4000|
|unadjusted |   4301| 4188| 4405|



We also plot each model's attack-size posterior median and 90% credible
interval against the realised truth.
We show intervals rather than densities here because the adjusted model's
posterior is pinned at the population cap, so a density would collapse to a
spike and hide the unadjusted model's spread.


``` r
ggplot(attack_summary, aes(x = median, y = model, colour = model)) +
  geom_pointrange(aes(xmin = q5, xmax = q95)) +
  geom_vline(
    aes(xintercept = sim$attack_size, linetype = "truth"), colour = "black"
  ) +
  scale_linetype_manual(values = c(truth = "dashed")) +
  labs(
    x = "Cumulative attack size", y = NULL, colour = NULL, linetype = NULL
  ) +
  theme_bw()
```

![Posterior median and 90% credible interval of the cumulative attack size under each model against the realised truth (dashed line).](figures/susceptible-depletion-plot-attack-1.png)

The adjusted model recovers the realised attack size, attributing the peak and
decline to a shrinking susceptible pool.
The unadjusted model has a constant reproduction number and no depletion, so it
cannot produce a peak and instead has to compromise between the growth and the
decline, biasing its reconstruction of the latent epidemic.
...
Table: Estimated population. True value: 4000

|variable        | mean|   q5|  q95|
|:---------------|----:|----:|----:|
|expr_pop_est[1] | 4086| 3986| 4185|



Rather than only reporting summary statistics, we plot the full posterior of the
estimated population against the true value used to simulate the data.
...
The depletion logic follows `EpiNow2` [@EpiNow2; @Abbott2020].
