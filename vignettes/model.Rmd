...
We provide a flexible semi-parametric model for the underlying transmission process similar to that implemented in other real-time modelling packages (such as `EpiNow2` and `epidemia`). This optionally includes a renewal process and latent reporting process with potentially group and time-varying generation times and reporting distributions. Howover, our default model contains minimal mechanism in order to more flexibly fit highly informative data.
...
In the following sections we provide further methodological and implementation details for the modular modelling framework implemented in `epinowcast`. Note that is framework has been designed to be extensible and so users are enabled to alter and extend our currently supported functionality.
...
We model the instantaneous reproduction number ($R_t$) on the log scale (tohugh other links will be supported in the future). When the generation time is fixed to be a day this can be interpreted as the instantaneous growth rate ($r_t$) defined as the difference in the log of the expected number of final notifications between time $t$ and $t-1$.
