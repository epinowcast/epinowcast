...
In our default model, we consider the delay to follow a discretised log-normal distribution as follows,
...
We generalise this model in order to support a range of delay distributions and reporting processes. Following the approach of Günther et al.[@gunther2021], we estimate the delay probability ($p_{g,t,d}$) via a discrete-time logistic hazard model
$$
...
where $W_{g,t,d}$ is a design matrix for different effects on the reporting delay. We extend the model of Günther et al. by decomposing $W_{g,t,d}$ into 3 components:
  ...
  - a hazard derived from a parametric delay distribution ($\gamma_{g,t,d}$) dependent on covariates at the time of reference
  - a hazard not derived from a parametric distribution ($\delta_{g,t,d}$) dependent on covariates at the time of reference
  - a hazard dependent on covariates referenced at the time of report ($\epsilon_{g,t,d}$).
