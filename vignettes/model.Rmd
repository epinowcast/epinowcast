...
For example, `enw_expectation(r = ~ 1 + rw(week))` implements a weekly random walk for the log reproduction number as described in equation (78).
...
For example, `enw_expectation(observation = ~ 1 + day_of_week)` models day-of-week variation in ascertainment as described in equation (101).
...
For example, `enw_reference(parametric = ~ 1 + (1 | age_group))` creates age group-specific random effects for both parameters in equations (162)-(163), allowing different age groups to have different delay distributions whilst sharing information through hierarchical pooling.
...
For example, `enw_reference(non_parametric = ~ 1 + day_of_week)` adds day-of-week effects to the reference date hazard in equation (171), whilst `enw_report(non_parametric = ~ 1 + day_of_week)` adds day-of-week effects to the report date hazard in equation (172).
...
# Summary of module-equation mappings

The following table summarises how module function arguments map to equations in the model definition:

| Module Function | Argument | Affects Equation(s) | Parameters Modified |
|-----------------|----------|---------------------|---------------------|
| `enw_reference()` | `parametric` | (162)-(163) | $\mu_{g,t}$, $\upsilon_{g,t}$ (distribution parameters) |
| `enw_reference()` | `non_parametric` | (174) | $\delta_{g,t,d}$ (reference date hazard component) |
| `enw_report()` | `non_parametric` | (175) | $\epsilon_{g,t,d}$ (report date hazard component) |
| `enw_expectation()` | `r` | (78) | $R_{g,t}$ (reproduction number/growth rate) |
| `enw_expectation()` | `observation` | (103) | $\nu_{g,t}$ (ascertainment) |
