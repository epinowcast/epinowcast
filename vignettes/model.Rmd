...
Each component adds to the overall hazard through a regression with logit link
\begin{equation}
...
The parametric baseline hazard $\gamma_{g,t,d}$ for a case in group $g$ with reference time $t$ is modelled according to a certain parametric probability distribution with parameters $\mu_{g,t}$ and $\upsilon_{g,t}$. By default, this is a discretised log-normal distribution, but other distributions are supported. To discretise the continuous probability distribution into discrete probabilities $p^{\prime}_{g,t,d}$, we use its cumulative distribution function $F^{\mu_{g,t}, \upsilon_{g,t}}$ to compute
...
The delay probabilities obtained from the discretised delay distribution are converted into hazards on the logit scale using
