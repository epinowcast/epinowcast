...
We generalise this model in order to support a range of delay distributions and effects of the reporting process on the delay. Following the approach of Günther et al.[@gunther2021], we parameterise the delay probability ($p_{g,t,d}$) via a discrete-time logistic hazard model
$$
h_{g,t,d} =\text{P} \left(\text{delay}=d|\text{delay} \geq d, W_{g,t,d}\right),
$$
where $W_{g,t,d}$ is a design matrix for different effects on the reporting delay. We extend the model of Günther et al. by decomposing $W_{g,t,d}$ into 3 components:

  - Parametric reference time model $\gamma_{g,t,d}$: a hazard derived from a parametric delay distribution whose parameters depend on covariates at the time of reference
...
The parametric reference time model $\gamma_{g,t,d}$ describes for group $g$ and reference time $t$ what the probability of reporting with delay $d$ was if it followed a certain parametric distribution (discretised log-normal by default, but other distributions are supported). The parameters of this distribution are $\mu_{g,t}$ (the log mean) and $\upsilon_{g,t}$ (the log standard deviation). They are defined using a (log-)linear model each. The model consists of an intercept and a number of arbitrary, shared covariates, indexed by reference time. The covariates are multiplied by fixed ($\beta_{f,i}$) and random ($\beta_{r,i}$) coefficients (note that these can include auto-regressive terms). Overall, we have
...
Currently, the parametric part of the delay distribution can be modeled by one of 4 distributions: (i) log-normal (default), (ii) exponential, (iii) gamma, and (iv) log-logistic. 

The table below provides a link to the definition of each parametric distribution, specifies how $\mu_{g,t}$ and $\nu_{g,t}$ as defined above are mapped to the parameters of the distribution (according to the referenced definition), and states what the resulting mean of the distribution (before discretization and adjustment for an assumed maximum delay).
...
For a specific reference date $t$ and maximum delay *max_delay*, the discretised parametric delay distribution is constructed as follows:
