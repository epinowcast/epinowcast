...
The first component ($\gamma_{gtd}$) we estimate what would be the probability of reporting $p^{\prime}_{gtd}$ on a given date if it followed a parametric distribution, here implemented using a discretised log normal (other forms might be implemented in the future) with the log mean and log standard deviation being defined using an intercept and arbitrary shared, occurrence date indexed, covariates with fixed ($\alpha_{i}$) and random ($\beta_{i}$) coefficients,
