...
The first component ($\gamma_{gtd}$) we estimate what would be the probability of reporting $p^{\prime}_{gtd}$ at a given time if it followed a parametric distribution, here implemented using a discretised log normal (other forms might be implemented in the future) with the log mean and log standard deviation being defined using an intercept and arbitrary shared, reference time indexed, covariates with fixed ($\alpha_{i}$) and random ($\beta_{i}$) coefficients,
