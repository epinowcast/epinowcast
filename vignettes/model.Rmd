...
We provide a flexible semi-parametric model for the underlying transmission process similar to that implemented in other real-time modelling packages [@EpiNow2; @epidemia]. This optionally includes a renewal process and latent reporting process with potentially group and time-varying generation times and reporting distributions. Combined with the appropriate generation time distribution this approach has been shown to correspond to a Susceptible-Exposed-Infected-Recovered (SEIR) model. Howover, our default model contains minimal mechanism in order to more flexibly fit highly informative data.
...
In the following sections we provide further methodological and implementation details for the modular modelling framework implemented in `epinowcast`. We present both our default model for each section as well as the extended generic model. Note that is framework has been designed to be extensible and so users are enabled to alter and extend our currently supported functionality.
...
In settings where data is sparse or where users want to understand the underlying generative process our flexible default model is likely not a good choice. In these settings our generic model offers a range of options that are context specific.

## Instantaneous reproduction number/growth rate

We model the instantaneous reproduction number ($R_t$) on the log scale (though other links will be supported in the future). When the generation time is fixed to be a day this can be interpreted as the instantaneous growth rate ($r_t$) defined as the difference in the log of the expected number of final notifications between time $t$ and $t-1$.
