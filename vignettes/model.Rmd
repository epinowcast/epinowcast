...
A secondary outcome can be modelled jointly with the primary series.
The expected secondary value is built from the expected primary observations $\lambda_{g,t}$ by convolving with a delay distribution, applying a log-scale ascertainment term, and combining the convolved and current contributions into either an incidence target (a scaled, delayed convolution) or a prevalence target (contributions accumulated and removed over time, e.g. bed occupancy).
Because both series are fitted together, uncertainty in the primary nowcast propagates into the secondary estimate.

The scaling is specified through the formula interface (`scale`), the delay as a probability mass function, the target structure via `enw_secondary_opts()`, and the observation likelihood via `family`.
The secondary series is currently modelled at its reference date; right-truncation and an independent reporting-delay nowcast of the secondary series itself are planned.
For implementation details see `enw_secondary()`.
...
| `enw_secondary()` | `scale`, `delay`, `secondary` | Secondary outcome as a scaled, delayed convolution of the primary expectation |

For additional implementation details and examples, see the documentation for each module function (`?enw_reference`, `?enw_report`, `?enw_expectation`, `?enw_secondary`) and the age-stratified nowcasting vignette (`vignette("germany-age-stratified-nowcasting")`).
