...
A common situation is data that are only updated once per week but are then reported with a daily resolution.
Here the underlying process is modelled daily, reports arrive on Wednesdays for current and past reference dates, and the day of the week is included as a random effect in the report model so the reporting cycle can be learned from the data.
...
coverage_table <- function(forecast_sample) {
  forecast_sample |>
    as_forecast_quantile(probs = c(0.05, 0.25, 0.5, 0.75, 0.95)) |>
    get_coverage(by = "model") |>
    as.data.table() |>
    (\(dt) dt[interval_range %in% c(50, 90)])() |>
    unique(by = c("model", "interval_range")) |>
    dcast(model ~ interval_range, value.var = "interval_coverage") |>
    setnames(c("50", "90"), c("coverage_50", "coverage_90"))
}

scored_weekly_input <- map_dfr(
  weekly_samples,
...
scored_weekly <- score(scored_weekly_input)
coverage_weekly <- coverage_table(scored_weekly_input)
```
...
scored_weekly |>
  ...
  merge(coverage_weekly, by = "model", sort = FALSE) |>
  kable()
```



|model                                        |   bias|  dss| crps| overprediction| underprediction| dispersion| log_score| mad| ae_median| se_mean| coverage_50| coverage_90|
|:--------------------------------------------|------:|----:|----:|--------------:|---------------:|----------:|---------:|---:|---------:|-------:|-----------:|-----------:|
|Weekly timestep                              | 0.4000| 12.0|  280|            190|          0.0033|       85.0|       6.6| 340|       380|  540000|   0.6666667|   0.6666667|
|Daily process, weekly reporting (fitted)     | 0.0057|  8.3|   30|             20|          2.9000|        7.4|       5.1|  31|        40|    3900|   0.3333333|   0.6666667|
|Daily process, weekly reporting (structural) | 0.6200|  8.3|   22|             11|          0.0000|       11.0|       5.0|  47|        33|    1400|   0.3333333|   1.0000000|
|Daily benchmark                              | 0.8900| 11.0|   36|             28|          0.0000|        7.9|       6.1|  34|        51|    2900|   0.0000000|   0.3333333|
...
<div class="figure">
<img src="figures/temporal-aggregation-weekly-comparison-plot-1.png" alt="Weekly-scale nowcasts from all four models on shared axes." width="100%" />
<p class="caption">Weekly-scale nowcasts from all four models on shared axes.</p>
</div>

All three daily-process models have CRPS in the tens at the weekly resolution while the pure weekly model sits at 280 — an order-of-magnitude gap driven by its `rw(week)` random walk having only four weekly observations to learn from, so when the series turns over at the end of the window it lacks the within-week structure needed to catch the reversal and instead extrapolates the recent rising trend.
Bias and coverage tell a more nuanced story than CRPS alone.
All daily-process models over-predict at this aggregation (positive bias) because slight over-prediction in each daily sample compounds when seven of them are summed.
Coverage should ideally match the nominal level, with both under- and over-coverage indicating miscalibration: the daily benchmark has the tightest intervals and under-covers severely (90 percent coverage of 33 percent), the fitted variant is closer to nominal, and the structural variant over-covers (its 90 percent interval contains every common week, indicating it is too wide given its mean prediction).
...
Daily samples are scored against `latest_obs_daily` over the daily reference dates the three models share.
...
scored_daily <- score(scored_daily_input)
coverage_daily <- coverage_table(scored_daily_input)
```
...
scored_daily |>
  summarise_scores(by = "model") |>
  summarise_scores(fun = signif, digits = 2, by = "model") |>
  merge(coverage_daily, by = "model", sort = FALSE) |>
  kable()
```
...
|model                                        |   bias| dss| crps| overprediction| underprediction| dispersion| log_score| mad| ae_median| se_mean| coverage_50| coverage_90|
|:--------------------------------------------|------:|---:|----:|--------------:|---------------:|----------:|---------:|---:|---------:|-------:|-----------:|-----------:|
|Daily process, weekly reporting (fitted)     | -0.360| NaN|  7.3|            1.5|             3.5|        2.3|      -Inf|  10|       9.7|     180|   0.4285714|   0.6785714|
|Daily process, weekly reporting (structural) | -0.140| NaN|  8.6|            2.3|             2.5|        3.8|       NaN|  16|      12.0|     600|   0.5000000|   0.7857143|
|Daily benchmark                              | -0.024| NaN|  5.7|            1.6|             1.6|        2.5|      -Inf|  11|       7.9|     120|   0.4642857|   0.8571429|
...
<div class="figure">
<img src="figures/temporal-aggregation-daily-comparison-plot-1.png" alt="Daily-scale nowcasts from the three daily-process models on shared axes." width="100%" />
<p class="caption">Daily-scale nowcasts from the three daily-process models on shared axes.</p>
</div>

The daily benchmark has the lowest daily CRPS, the smallest bias, and the best 90 percent coverage, as expected from the model with access to the full daily data.
The two weekly-reporting variants are close behind on CRPS, slightly more negatively biased, and only modestly worse on coverage.
This is the pay-off for using a daily latent process with a known reporting cycle: even when reporting only happens once a week, the daily-scale nowcasts barely degrade compared with a fully daily fit.

All three daily models under-cover at the 90 percent level — around 70-85 percent rather than the nominal 90 — with the latest data points sitting a little above their upper credible bands.
Both under- and over-coverage signal miscalibration; here the issue is intervals that are slightly too tight, most likely a misspecification of the reporting delay rather than a problem with the temporal scaffold: we are using the default lognormal delay with no temporal variation, and a non-parametric or time-varying delay (or an explicit day-of-week effect on the reporting hazard) would give the model the flexibility to track the lengthening tail.
The [getting started vignette](epinowcast.html) and `?enw_reference` cover the available options.
...
Comparable runtimes show how much computation each approach buys you.
...
|:--------------------------------------------|------------:|
|Weekly timestep                              |          0.7|
|Daily process, weekly reporting (fitted)     |        240.0|
|Daily process, weekly reporting (structural) |         94.0|
|Daily benchmark                              |        160.0|
