...
|:--------------------------------------------|------:|----:|----:|--------------:|---------------:|----------:|---------:|---:|---------:|-------:|
|Weekly timestep                              | 0.4000| 12.0|  280|            190|          0.0033|       85.0|       6.6| 340|       380|  540000|
|Daily process, weekly reporting (fitted)     | 0.0057|  8.3|   30|             20|          2.9000|        7.4|       5.1|  31|        40|    3900|
|Daily process, weekly reporting (structural) | 0.6200|  8.3|   22|             11|          0.0000|       11.0|       5.0|  47|        33|    1400|
|Daily benchmark                              | 0.8900| 11.0|   36|             28|          0.0000|        7.9|       6.1|  34|        51|    2900|
...
All three daily-process models score similarly at the weekly resolution and track the truth tightly on the common weeks.
The pure weekly model is wider and more biased: its `rw(week)` random walk has only four weekly observations to learn from, so when the underlying series turns over at the end of the window the model lacks the within-week structure needed to catch the reversal and instead extrapolates the recent rising trend.
This particular slice of data makes the gap look large; the cost of working purely at the weekly scale will be smaller on flatter or more slowly varying series and larger near turning points.
...
|model                                        |   bias| dss| crps| overprediction| underprediction| dispersion| log_score| mad| ae_median| se_mean|
|:--------------------------------------------|------:|---:|----:|--------------:|---------------:|----------:|---------:|---:|---------:|-------:|
|Daily process, weekly reporting (fitted)     | -0.360| NaN|  7.3|            1.5|             3.5|        2.3|      -Inf|  10|       9.7|     180|
|Daily process, weekly reporting (structural) | -0.140| NaN|  8.6|            2.3|             2.5|        3.8|       NaN|  16|      12.0|     600|
|Daily benchmark                              | -0.024| NaN|  5.7|            1.6|             1.6|        2.5|      -Inf|  11|       7.9|     120|
...
The structural and fitted weekly-reporting models track the daily benchmark closely, both visually and on daily CRPS.
...
All three daily models slightly under-cover the most recent reference dates, where the latest data points sit a little above the upper credible band.
...
runtimes <- data.table(
  ...
  `Run time (s)` = signif(c(
    nowcast_weekly$run_time,
    nowcast_weekly_rep$run_time,
    nowcast_weekly_rep_structural$run_time,
    nowcast_daily$run_time
  ), 2)
)
kable(runtimes)
```
...
Among the daily models the structural variant is fastest because pinning the reporting cycle removes the day-of-week random effect on the report side; the fitted variant is the slowest as it has to learn that effect under sparse Wednesday-only observations and uses a lower `adapt_delta`; the daily benchmark sits between them.
