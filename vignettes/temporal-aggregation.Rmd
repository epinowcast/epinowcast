...
We then restrict scoring to the weeks every model has predictions for so the comparison is on a common set of reference weeks.
...
common_weeks <- Reduce(
  intersect, lapply(samples_list, function(s) unique(s$reference_week))
)
common_weeks <- intersect(common_weeks, weekly_truth$reference_week)
common_weeks <- as.Date(common_weeks)
