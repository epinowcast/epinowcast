...
The fitted variant has the simplest likelihood per evaluation of the two remaining models — the LOCF step function in `confirm` is highly redundant — but it has to explore a wider posterior over the day-of-week random effects, so its run time and the daily benchmark's are similar and the ordering between them depends on Stan's run-to-run variability.
