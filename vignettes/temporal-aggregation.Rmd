...
interval_coverage <- function(samples_truth, by) {
  samples_truth[
    ...
    , .(coverage_50 = mean(observed >= q25 & observed <= q75),
        coverage_90 = mean(observed >= q5 & observed <= q95)),
    by = "model"
  ]
}
...
scored_weekly <- scored_weekly_input |>
  as_forecast_sample(
    observed = "observed", predicted = "sample", sample_id = ".draw"
  ) |>
  score()
coverage_weekly <- interval_coverage(
  as.data.table(scored_weekly_input), by = c("model", "reference_week")
)
```
...
The structural variant is the most reliable on coverage — its 90 percent interval contains every common week — while the daily benchmark has the tightest intervals and consequently the worst coverage despite a similar CRPS, because its weekly sums sit consistently above the truth.
The fitted variant is the least biased of the three on bias but covers roughly as well as the weekly model.
...
scored_daily <- scored_daily_input |>
  as_forecast_sample(
    observed = "observed", predicted = "sample", sample_id = ".draw"
  ) |>
  score()
coverage_daily <- interval_coverage(
  as.data.table(scored_daily_input), by = c("model", "reference_date")
)
```
...
All three daily models under-cover at the 90 percent level (around 70-85 percent rather than the nominal 90 percent), with the latest data points sitting a little above their upper credible bands.
This is most likely a misspecification of the reporting delay rather than a problem with the temporal scaffold: we are using the default lognormal delay with no temporal variation, and a non-parametric or time-varying delay (or an explicit day-of-week effect on the reporting hazard) would give the model the flexibility to track the lengthening tail.
