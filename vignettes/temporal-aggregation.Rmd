...
Here the underlying process is modelled daily, reports arrive on Wednesdays for current and past reference dates, and the day of the week is included as a random effect in the report model.
This is the natural starting point if the reporting cycle is unknown but it is poorly identified when reporting is concentrated on a single weekday because only one day-of-week level enters the likelihood; the structural variant in the next section pins the cycle and is generally preferable for genuinely once-a-week reporting.
...
``` r
plot(nowcast_weekly_rep, latest_obs = latest_obs_weekly)
```
...
Approaches 2, 3, and 4 produce daily nowcasts that we summarise to weekly counts by summing daily samples into the same weekly bins that `enw_aggregate_cumulative()` produces.
By construction every model covers the same nowcast horizon so the comparison is on a common set of reference weeks.
...
The pure weekly model has the best weekly CRPS — it is fit directly at this resolution and avoids any reporting-cycle complication.
The structural variant of the daily-process model matches the daily benchmark on weekly CRPS, showing that a daily latent process with a known weekly reporting cycle can recover the same predictive performance as a fully daily fit.
The fitted variant scores much worse: with `observation_indicator = ".observed"` only Wednesday rows enter the likelihood so the day-of-week random effect on the report side has data only for one of seven levels.
The remaining six levels stay close to their priors and the model's predicted cumulative on non-Wednesday days is dominated by that prior, inflating the daily-scale nowcast samples and the weekly sum derived from them.
...
- Use a **daily process with weekly reporting** when daily inference is the goal but reporting only happens once a week.
  Prefer the structural variant when reporting is genuinely concentrated on a known weekday: it pins the reporting cycle and keeps the daily-scale predictions on the right scale.
  A fitted day-of-week reporting effect is only useful if reporting is genuinely spread across multiple days; with single-weekday reporting it is poorly identified.
