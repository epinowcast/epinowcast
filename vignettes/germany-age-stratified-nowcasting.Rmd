...
```r
options(mc.cores = 2)
simple_nowcast <- epinowcast(pobs,
  model = model,
  save_warmup = FALSE,
...
```r
plot(simple_nowcast, obs = latest_nat_germany) +
...
As noted using the posterior predictions from the simple model fit above there appears to be a day of the week effect for reported observations. To adjust for this we introduce a random effect for day of the week by date of report using the following helper function.
...
age_reference_effects <- enw_formula(
  pobs$metareference, random = "age_group"
)
...
metareference <- enw_add_cumulative_membership(
  pobs$metareference[[1]], feature = "week"
)
metareference
```
...
week_age_reference_effects <- enw_formula(
  metareference, random = "age_group", custom_random = "cweek"
)
...
```r
nowcasts <- list(simple = simple_nowcast, dow = dow_nowcast, age = age_nowcast,
...
summmarised_nowcasts <- map(
  nowcasts, summary, probs = c(0.025, 0.5, seq(0.1, 0.9, by = 0.1), 0.95, 0.975)
)
