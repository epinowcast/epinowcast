...
```{r, child=c('../man/chunks/_readme-install-epinowcast.Rmd')}
```
...
High values of `r_hat`, the Gelman-Rubin statistic used to measure convergence, indicate a lack of convergence between the different model chains.
...
Similarly, lower values for the [effective sample size](https://mc-stan.org/docs/reference-manual/effective-sample-size.html) are especially problematic.
...
The Stan community is a pretty warm and open place and receptive to helping others.
