...
We also keep the eventually-observed counts for the same reference dates so each nowcast can be shown against the data that arrived later.


``` r
latest_obs <- enw_latest_data(nat_germany_hosp)[
  reference_date %in% unique(retro$reference_date)
]
```
...
|variable            |   mean|     q5|   q95|
|:-------------------|------:|------:|-----:|
|expr_arima_pacf[1]  | -0.351| -0.940| 0.493|
|expr_arima_sigma[1] |  0.018|  0.005| 0.039|
...
``` r
plot(nowcast_ma, latest_obs = latest_obs) +
  facet_wrap(vars(age_group), scales = "free_y")
```
...
This higher-order moving-average model is the one term here that the short chains do not fully resolve (see the diagnostics below): without an enforced invertibility constraint the MA(q) likelihood is multimodal, so the chains can settle on different but observationally similar coefficient sets.
Lowering `q` or running longer chains sharpens it; the [implementation notes](arima.html) cover the invertibility trade-off.
...
fits <- list(
  ...
  "arima(day, age_group, 1, 1, 0)" = nowcast_arima,
  "ma(day, age_group, 6)" = nowcast_ma,
...
|:------------------------------|--------:|------------:|-----------:|
|rw(week)                       |    1.011|          291|           2|
|(1 &#124; week:.group)         |    1.013|          161|           0|
|arima(day, age_group, 1, 1, 0) |    1.015|          306|           0|
|ma(day, age_group, 6)          |    1.191|            7|           0|
|(1 &#124; day_of_week)         |    1.013|          194|           0|
