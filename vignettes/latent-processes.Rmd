...
- **Gaussian process** via `gp(week)`. A smooth function of time with a length scale that controls how quickly the growth rate can change; a flexible non-parametric smoother.
...
Older age groups tend to report more slowly, so a single shared delay distribution misfits the groups whose reporting differs from the average.
We let the delay vary by age group with a shared random effect and reuse the same reference model for every fit, so the comparison stays focused on the growth-rate choice rather than the delay.
...
Differencing is applied via the cumulative-sum operator and the ARMA part via a parameter-dependent [Toeplitz kernel](arima.html) built from the impulse response; both compose with a single matrix multiplication onto unit-normal shocks.
...
![Nowcast under an ARIMA(1, 1, 0) residual on week.](figures/latent-processes-arima-plot-1.png)
...
|variable            |  mean|     q5|   q95|
|:-------------------|-----:|------:|-----:|
|expr_arima_pacf[1]  | 0.000| -0.813| 0.845|
|expr_arima_sigma[1] | 0.029|  0.010| 0.054|
...
![Nowcast under a stationary AR(1) residual on day.](figures/latent-processes-ar-plot-1.png)
...
By default the process is stationary; `gp(week, d = 1)` integrates it once for a smoothly drifting, random-walk-like trend (mirroring `arima()`'s `d`), as detailed in the [implementation notes](gaussian-process.html#differencing).
...
The [Gaussian process implementation notes](gaussian-process.html) cover the spectral approximation, the kernels, and the priors in detail.
...
![Nowcast under a Gaussian process on weeks.](figures/latent-processes-gp-plot-1.png)
...
|variable         |  mean|    q5|   q95|
|:----------------|-----:|-----:|-----:|
|expr_gp_rho[1]   | 3.122| 1.283| 6.104|
|expr_gp_alpha[1] | 0.046| 0.004| 0.104|
...
The `(1 | group)` notation follows the same random-effect convention as [`lme4`](https://cran.r-project.org/package=lme4) and [`brms`](https://paulbuerkner.com/brms/).
...
Latent and periodic terms compose in the same formula, which is the usual surveillance model: a slowly evolving trend plus a repeating weekly pattern.
Here an integrated AR term carries the trend at weekly resolution while a day-of-week effect captures the within-week reporting cycle.
Each term is identified by what it explains — the trend by the latent series, the weekday structure by the periodic effect — so they can be read off separately in the fit.
...
![Nowcast combining a day-of-week effect with an ARIMA(1, 1, 0) trend on week.](figures/latent-processes-combined-plot-1.png)
...
Higher-order moving averages are harder to fit: without an enforced invertibility constraint the MA(q) likelihood is multimodal, so chains can settle on different but observationally similar coefficient sets.
The [implementation notes](arima.html) cover the invertibility trade-off.
...
|:-----------------------------------------|--------:|------------:|-----------:|---------:|
|rw(week)                                  |    1.019|          164|           0|       117|
|arima(week, age_group, 1, 1, 0)           |    1.008|          188|           0|        49|
|ar(day, age_group, 1)                     |    1.020|          172|           0|        70|
|gp(week)                                  |    1.021|          136|           0|        54|
|(1 &#124; week:.group)                    |    1.013|          188|           0|        41|
|(1 &#124; day_of_week)                    |    1.019|          171|           0|       162|
|(1 &#124; day_of_week) + arima(week, ...) |    1.023|          218|           0|       170|
...
| Gaussian process | Smooth, learned length scale | `gp(week)` | You want a flexible non-parametric smoother whose smoothness is learned from the data rather than fixed by the differencing order. |
...
The same `rw()`, `ar()`, `arima()`, `gp()`, and random-effect terms can be placed on any module's formula: an `arima()` term on the reference delay mean, for instance, models a reporting delay that drifts over time.
See the [implementation notes](arima.html) for the full list of modules that accept them.
