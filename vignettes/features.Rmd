...
|----------|-------------|----------------|---------|
| Parametric only | Well-characterised delays | `enw_reference(parametric = ~1, distribution = "lognormal")` | Default in vignettes |
| Non-parametric only | Unknown delay distribution | `enw_reference(parametric = ~0, non_parametric = ~1 + (1 | delay))` | Flexible hazard model |
| Mixed model | Parametric baseline + adjustments | `enw_reference(parametric = ~1, non_parametric = ~0 + (1 | delay_cat))` | Best of both |
...
**Key functions:**
- `enw_expectation()`: Specify latent process model
...
Handle observations with missing reference dates.

| Feature | What it does | How to use | Status |
|---------|--------------|------------|--------|
| Missing reference model | Models proportion of missing | `enw_missing(formula = ~1)` | Experimental |
| Assumes consistent delays | Same delay for missing and observed | Automatic when missing data present | Limitation |

**Key functions:**
- `enw_missing()`: Specify missing data model

**Where to see it:** See `?enw_missing` for details.
...
|---------|--------------|----------------|-------------|
| Within-chain threading | Parallel likelihood calculation | `threads_per_chain = 4` | Large datasets |
...
1. **Prepare data:** `enw_preprocess_data()`
2. **Specify models:**
   - Delays: `enw_reference()`
   - Report effects: `enw_report()`
   - Latent process: `enw_expectation()`
   - Observation: `enw_obs()`
3. **Fit:** `epinowcast()`
4. **Evaluate:** `plot()`, `summary()`

**Where to start:** The [getting started vignette](epinowcast.html) walks through this workflow step-by-step.
