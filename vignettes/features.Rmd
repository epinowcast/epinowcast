...
| **Secondary observations** | Joint primary -> secondary models (e.g. deaths from cases, bed occupancy from admissions) | [Secondary observations](#secondary-observations) |
...
| **Retrospective Rt** | Rt estimation from finalised counts without delay modelling | [Rt estimation vignette](single-timeseries-rt-estimation.html) |
...
| **Inference methods** | NUTS, pathfinder, pathfinder-initialised NUTS | [Inference methods vignette](inference-methods.html) |
...
| Weekly | Reduced computational cost | `timestep = "week"` | [Temporal aggregation guide](temporal-aggregation.html) |
| Daily process, weekly reporting | Daily inference from once-a-week reporting | `timestep = "day"` with weekly observation scaffold | [Temporal aggregation guide](temporal-aggregation.html) |
...
- `enw_rolling_sum()` and `enw_dayofweek_structural_reporting()`: Build the daily-process / weekly-reporting scaffold

**Where to see it:** The [getting started vignette](epinowcast.html) uses daily data.
For weekly data, weekly reporting on a daily process, and a side-by-side comparison with the daily benchmark see the [temporal aggregation guide](temporal-aggregation.html).
...
# Report Date Effects and Structural Reporting {#report-date-effects}
...
| Custom structural patterns | Complex reporting schedules | `enw_structural_reporting_metadata()` with custom data | Flexible aggregation |

**Key functions:**

- `enw_report()`: Specify report date model
- `enw_dayofweek_structural_reporting()`: Helper for day-of-week patterns
- `enw_structural_reporting_metadata()`: Create custom structural reporting metadata
- `enw_rolling_sum()`: Aggregate observations over timesteps
- See `?enw_report` for details

**Where to see it:** See the [temporal aggregation guide](temporal-aggregation.html) for worked examples of weekly reporting on a daily process model, with both fitted and structural day-of-week reporting.
...
**Where to see it:** The [latent processes vignette](latent-processes.html) compares random walks, ARIMA(p, d, q) residuals, stationary AR, and periodic terms on the growth rate, with the [ARIMA reference](arima.html) covering the maths, priors, and usage. The [Rt estimation vignette](single-timeseries-rt-estimation.html) demonstrates renewal process models with generation times.

# Secondary Observations {#secondary-observations}

Model a secondary outcome as a function of the primary series, fitted jointly so the primary nowcast uncertainty propagates into the secondary estimate.
The expected secondary value is a delayed, scaled convolution of the primary expectation, combined into an incidence or a prevalence target.

| Target | Use case | How to specify | Example |
|--------|----------|----------------|---------|
| Incidence | Delayed counts (e.g. deaths from cases) | `enw_secondary_opts("incidence")` | Scaled convolution of the primary series |
| Prevalence | Accumulating counts (e.g. bed occupancy) | `enw_secondary_opts("prevalence")` | Inflow accumulated, outflow removed over time |
| Scaling | Ascertainment / case fatality ratio | `scale = ~1` (or any formula) | Formula interface, as for other modules |
| Delay | Primary-to-secondary reporting delay | `delay = c(0.2, 0.5, 0.3)` | Probability mass function |
| Observation family | Count likelihood | `family = "poisson"` / `"negbin"` | Matches `enw_obs()` |

**Key functions:**

- `enw_secondary()`: Specify the secondary observation model and supply secondary counts via `obs`
- `enw_secondary_opts()`: Choose the incidence or prevalence target
- Pass the configured module to `epinowcast(..., secondary = ...)`

```r
secondary <- enw_secondary(
  secondary = enw_secondary_opts("incidence"),
  obs = deaths, scale = ~1, delay = c(0.2, 0.5, 0.3),
  family = "poisson", data = pobs
)
nowcast <- epinowcast(pobs, secondary = secondary)
```

The scaling is specified through the formula interface, so it can vary by covariates or group exactly as the other modules' effects do (e.g. `scale = ~ 1 + (1 | .group)`).
Right-truncation and an independent reporting-delay nowcast of the secondary series itself are not yet supported; the primary series retains its full nowcasting model.
...
- `ar()`, `arima()`: Autoregressive and ARIMA latent residual helpers
...
Each model module defines default normal priors which can be inspected and customised.
To set custom priors, pass a `data.frame` with columns `variable`, `mean`, and `sd` to the `priors` argument of `epinowcast()`.
...
- `enw_replace_priors()`: Merge custom priors with defaults
- See `?epinowcast` for a worked example
...
| Scoring rules | Probabilistic forecast evaluation | `scoringutils` package | [Germany hierarchical vignette](germany-age-stratified-nowcasting.html) |
...
`plot()` methods are available for both preprocessed data and nowcast output.
...
- `plot()`: S3 methods for both object types
- `enw_plot_delay_cumulative()`, `enw_plot_delay_fraction()`, `enw_plot_delay_quantiles()`, `enw_plot_delay_counts()`: Individual plot functions for preprocessed data
- `enw_plot_nowcast_quantiles()`, `enw_plot_pp_quantiles()`: Individual plot functions for model output
- `enw_delay_categories()`, `enw_delay_quantiles()`: Helper functions for computing delay summaries

**Where to see it:** The [visualising preprocessed data vignette](preprocess-visualisation.html) demonstrates all preprocessing plot types.
...
**Where to see it:** The [inference methods vignette](inference-methods.html) compares NUTS, pathfinder, and pathfinder-initialised NUTS with runtime and posterior comparisons. See also the [Stan help vignette](stan-help.html) for guidance on computational settings.
...
|------|-----------|---------|
| Convert from line list | `enw_linelist_to_incidence()` | Individual → aggregate data |
...
| Change timesteps | `enw_aggregate_cumulative()` | Daily → weekly, etc. |
...
The following features are not currently supported but may be of interest.
If you need any of these capabilities, please reach out via our [community forum](https://community.epinowcast.org/) or [GitHub discussions](https://github.com/epinowcast/epinowcast/discussions).
...
| **Retrospective Rt estimation** | Supported | Use `max_delay = 1` or `enw_retrospective()` to skip delay modelling and estimate Rt from finalised counts. See the [Rt estimation vignette](single-timeseries-rt-estimation.html) |
...
**Get involved:** We welcome contributions and discussions about extending the package to support these features.
See our [community forum](https://community.epinowcast.org/) for ongoing discussions.
...
- [Visualising preprocessed data](preprocess-visualisation.html): Explore delay structure before fitting
