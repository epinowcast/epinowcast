...
**Available distributions:** See the [distributions vignette](distributions.html) for details on lognormal, gamma, exponential, and loglogistic distributions.
...
Handle observations with missing reference dates and extend data for forecasting.
...
| Forecasting | Extend data with missing observations | `enw_extend_date()` + `enw_missing()` | Combine with missing data model |
...
- `enw_extend_date()`: Extend time series for forecasting

**Where to see it:** See `?enw_missing` and `?enw_extend_date` for details.
