...
::: {.alert .alert-warning}
**Important:** The model specifications in this vignette are simplified
examples for demonstration.
The default lognormal parametric reporting delay distribution may not be
appropriate for your data.
It can fail when reporting delays have multiple modes or complex patterns
(see issue [#319](https://github.com/epinowcast/epinowcast/issues/319)
for examples).
Users should evaluate model fit using posterior predictive checks and
consider alternative specifications such as non-parametric hazard models
when the lognormal distribution does not adequately capture the reporting
delay structure.
See the [model definition vignette](https://package.epinowcast.org/articles/model.html)
for detailed guidance on model specification.
:::

In the following sections, we specify simple models for each of these modules.
The appropriateness of these specifications will vary depending on your context.
See our vignettes for further details on model specification and examples of more complex models.
...
lognormally distributed, and static over time and strata.
We can specify this model using the `enw_reference()` function,
...
*Note: The default distribution is lognormal, hence the distribution
argument could be omitted here.*
However, this parametric assumption may not be appropriate if your
reporting delay distribution has multiple modes or non-lognormal
characteristics.
Always check model fit using posterior predictive checks
(see [Plotting posterior predictions](#plotting-posterior-predictions)
below).

Alternatively we could model the reporting delay non-parametrically using
a hazard model (see [our model description for details](https://package.epinowcast.org/articles/model)).
The following is equivalent to a cox proportional hazards model with a
single baseline hazard function.
