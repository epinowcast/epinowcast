...
| Laplace | Fast (optimise then sample at the mode) | Approximate; Gaussian posteriors that tend to be narrow | Limited (optimiser convergence) | Quick checks, model development |

**Practical guidance:**

- Start with **pathfinder** during model development to iterate quickly on specification choices.
- Consider **Laplace** (`enw_laplace()`) as an alternative fast approximation. It optimises to the posterior mode and samples from a Gaussian there, so like pathfinder it tends to underestimate tail variance and is best for quick checks rather than final inference.
