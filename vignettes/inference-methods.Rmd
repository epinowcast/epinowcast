...
We tested a range of `adapt_delta` values (0.80 to 0.95) for pathfinder-initialised NUTS and found that a lower setting of 0.90 was sufficient when the sampler starts from pathfinder's estimate of the typical set.
This reduces warmup cost without increasing divergent transitions for these models.


``` r
nuts_pf_args <- nuts_args
nuts_pf_args$adapt_delta <- 0.90
...
Pathfinder uses Pareto Smoothed Importance Sampling (PSIS) to resample from the variational approximation.
CmdStan reports Pareto k diagnostics for this step.
High Pareto k values (> 0.7) indicate that the importance resampling was unable to adequately correct the approximation.
We can extract these diagnostics from the underlying CmdStan fit.
...
pf_diag <- rbindlist(lapply(names(pf_fits), function(nm) {
  fit_obj <- pf_fits[[nm]]$fit[[1]]
  lw <- fit_obj$lp()
  data.table(
    Label = nm,
    `Draws` = length(lw),
    `Min lp__` = round(min(lw), 1),
    `Median lp__` = round(median(lw), 1),
    `Max lp__` = round(max(lw), 1)
  )
}))

knitr::kable(
  pf_diag, digits = 2,
  caption = "Pathfinder fit summaries."
)
```



Table: Pathfinder fit summaries.

|Label         | Draws| Min lp__| Median lp__| Max lp__|
|:-------------|-----:|--------:|-----------:|--------:|
|A: Pathfinder |  1000|  -1601.1|     -1591.5|  -1590.2|
|B: Pathfinder |  1000|  -1650.2|     -1637.9|  -1633.1|



High Pareto k warnings (printed during fitting) are common for complex hierarchical models like these and reinforce the guidance that pathfinder results should be validated against NUTS for final inference.
