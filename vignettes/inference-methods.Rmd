...
description: "A comparison of NUTS sampling, pathfinder and Laplace approximate inference, and pathfinder-initialised NUTS across two model specifications."
...
- **Laplace** (`enw_laplace()`): a fast Laplace approximation that optimises to
  the posterior mode and then draws from a Gaussian centred there.
  Like pathfinder, it is suited to model development and quick checks.
...
This vignette compares these approaches across two nowcasting model specifications of different complexity.
...
## Laplace (approximate inference)

The Laplace method optimises the model to the posterior mode and then draws from a Gaussian approximation centred there.
We draw the same number of approximate samples as the other methods and set a seed, though the optimisation and draw steps are not fully deterministic.


``` r
lap_opts <- enw_fit_opts(
  sampler = enw_laplace, pp = TRUE, draws = 1000, seed = 123
)

fit_a_lap <- fit_nowcast(reference = ref_a, fit = lap_opts)
# Spec B sits at the edge of what Laplace can handle: across repeated runs the
# optimiser sometimes reaches a usable mode and sometimes lands where the
# Gaussian draws produce non-finite quantities, so we wrap the fit and keep the
# comparison going either way rather than letting a failure stop the vignette.
fit_b_lap <- tryCatch(
  fit_nowcast(reference = ref_b, fit = lap_opts),
  error = function(e) conditionMessage(e)
)
fit_b_lap_ok <- inherits(fit_b_lap, "epinowcast")
```

On this run Laplace also produced a usable fit for spec B. As the comparisons below show, a usable fit does not mean its uncertainty is trustworthy.

Laplace fits spec A reliably.
Spec B has a flatter, more correlated posterior, and Laplace is not reliable there: across repeated runs the optimiser sometimes reaches a usable mode and sometimes lands where the Gaussian draws produce non-finite quantities, even with a fixed seed, because the optimisation and draw steps are not fully deterministic.
When the draws are unusable `enw_laplace()` aborts with an actionable error rather than failing later in an opaque way, which is itself a useful signal that the posterior is poorly approximated by a single Gaussian and that NUTS (or pathfinder-initialised NUTS) is needed.
As with pathfinder, a usable fit does not guarantee a good one; the practical check is agreement with the NUTS reference, shown below.
...
# Drop any methods that did not produce a fit (spec B Laplace on runs where it
# failed), keeping only objects that are actual fits.
fits <- Filter(function(x) inherits(x, "epinowcast"), fits)
...
|:-----------------------|:----------------------|-----------:|:---------|
|A (static delays)       |NUTS (prior init)      |       110.1|1.9       |
|A (static delays)       |NUTS (pathfinder init) |        97.2|1.9       |
|A (static delays)       |Pathfinder             |        18.4|-         |
|A (static delays)       |Laplace                |         0.6|-         |
|B (time-varying delays) |NUTS (prior init)      |       590.4|0.4       |
|B (time-varying delays) |NUTS (pathfinder init) |       543.7|0.1       |
|B (time-varying delays) |Pathfinder             |       134.8|-         |
|B (time-varying delays) |Laplace                |         1.9|-         |
...
|:-------------------------|-------:|--------:|---------------------:|--------------:|
|A: NUTS (prior init)      |    1000|     1.01|                    10|             10|
|A: NUTS (pathfinder init) |    1000|     1.02|                     1|             10|
|B: NUTS (prior init)      |    1000|     1.02|                     5|             10|
|B: NUTS (pathfinder init) |    1000|     1.04|                    10|             10|
...
The Laplace method has no MCMC diagnostics; its quality depends on the optimiser reaching the mode and on how Gaussian the posterior is in the unconstrained space.
The practical check is agreement with the NUTS reference, shown in the nowcast and posterior comparisons below.
Like pathfinder, it approximates the whole posterior with a single Gaussian, so it shares the tendency to underestimate posterior variance for skewed or multimodal posteriors.
It fits spec A reliably, and where it also fits spec B a usable fit still does not mean its uncertainty is trustworthy, as the comparisons below show.
Spec B is unreliable: across runs Laplace alternates between a usable fit and a clean failure, and when the draws are unusable `enw_laplace()` aborts with an actionable error, which is itself a useful signal that NUTS (or pathfinder-initialised NUTS) is needed.
...
For spec A, all methods produce similar nowcasts with good coverage of the observed data, suggesting that the approximate methods can be acceptable for simpler models despite high Pareto k values.
For spec B, pathfinder diverges more from NUTS than it does for spec A, reflecting the increased difficulty of approximating the time-varying delay posterior and resulting in less reliable coverage; Laplace appears here only when it produced a usable fit for spec B on this run.
...
|:-----------------------|:----------------------|:----------------|----:|-------------:|
|A (static delays)       |NUTS (prior init)      |refp_mean_int[1] | 2.45|          1.66|
|A (static delays)       |NUTS (prior init)      |refp_sd_int[1]   | 3.44|          0.70|
|A (static delays)       |NUTS (prior init)      |sqrt_phi[1]      | 1.50|          0.24|
|A (static delays)       |NUTS (pathfinder init) |refp_mean_int[1] | 2.44|          1.54|
|A (static delays)       |NUTS (pathfinder init) |refp_sd_int[1]   | 3.44|          0.64|
|A (static delays)       |NUTS (pathfinder init) |sqrt_phi[1]      | 1.50|          0.25|
|A (static delays)       |Pathfinder             |refp_mean_int[1] | 2.99|          0.17|
|A (static delays)       |Pathfinder             |refp_sd_int[1]   | 3.69|          0.13|
|A (static delays)       |Pathfinder             |sqrt_phi[1]      | 1.43|          0.13|
|A (static delays)       |Laplace                |refp_mean_int[1] | 2.33|          1.80|
|A (static delays)       |Laplace                |refp_sd_int[1]   | 3.40|          0.73|
|A (static delays)       |Laplace                |sqrt_phi[1]      | 1.46|          0.25|
|B (time-varying delays) |NUTS (prior init)      |refp_mean_int[1] | 2.27|          1.95|
|B (time-varying delays) |NUTS (prior init)      |refp_sd_int[1]   | 3.05|          1.46|
|B (time-varying delays) |NUTS (prior init)      |sqrt_phi[1]      | 1.31|          0.24|
|B (time-varying delays) |NUTS (pathfinder init) |refp_mean_int[1] | 2.21|          2.00|
|B (time-varying delays) |NUTS (pathfinder init) |refp_sd_int[1]   | 3.08|          1.54|
|B (time-varying delays) |NUTS (pathfinder init) |sqrt_phi[1]      | 1.31|          0.26|
|B (time-varying delays) |Pathfinder             |refp_mean_int[1] | 2.50|          0.30|
|B (time-varying delays) |Pathfinder             |refp_sd_int[1]   | 1.94|          0.12|
|B (time-varying delays) |Pathfinder             |sqrt_phi[1]      | 1.30|          0.10|
|B (time-varying delays) |Laplace                |refp_mean_int[1] | 2.10|          2.31|
|B (time-varying delays) |Laplace                |refp_sd_int[1]   | 2.07|          2.48|
|B (time-varying delays) |Laplace                |sqrt_phi[1]      | 1.22|          0.23|



Across all three parameters the point estimates (means) are broadly aligned between NUTS and the approximate methods.
...
Laplace is also a mode-centred Gaussian approximation and, where it produced a usable fit, shows the same tendency to produce intervals narrower than NUTS, as the table makes concrete.
This is a known limitation of these approximations: they concentrate on a mode and underestimate tail variance, so uncertainty from a raw pathfinder or Laplace fit is not trustworthy even when the central estimates look reasonable.
NUTS with pathfinder initialisation reproduces the NUTS posterior (widths within a few percent of prior-init NUTS), so the initialisation itself does not carry the approximation bias forward.
These numbers are specific to this model and data — run the same check on your own setup before relying on approximate posteriors.
...
#>         variable        mean         sd
#>           <char>       <num>      <num>
#> 1:    expr_r_int -0.01967519 0.11980538
#> 2:  expr_beta_sd  0.40953112 0.13884498
#> 3: refp_mean_int  2.44720615 0.52888283
#> 4:   refp_sd_int  3.44168896 0.22170571
#> 5:   rep_beta_sd  0.41885564 0.14706048
#> 6:      sqrt_phi  1.50265230 0.07547228
```
...
|Method                            | Runtime (s)|
|:---------------------------------|-----------:|
|Pathfinder (informed priors)      |         8.5|
|Pathfinder (default priors)       |        16.0|
|NUTS (default priors, full refit) |        84.0|
...
| Laplace | Fast (optimise then sample at the mode) | Approximate; Gaussian posteriors that tend to be narrow, and can fail outright on harder models | Limited (optimiser convergence) | Quick checks on simpler models |

**Practical guidance:**

- Start with **pathfinder** during model development to iterate quickly on specification choices.
- Consider **Laplace** (`enw_laplace()`) as an alternative fast approximation. It optimises to the posterior mode and samples from a Gaussian there, so like pathfinder it tends to underestimate tail variance and is best for development and quick checks rather than final inference. On harder models the optimiser can fail to reach a usable mode or the Gaussian can sample degenerate regions; `enw_laplace()` then aborts with an actionable error, a useful signal that the posterior is poorly approximated by a single Gaussian and that NUTS is needed.
