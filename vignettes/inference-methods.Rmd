...
(prior init)      |        69.6|1.6       |
|A (static delays)       |NUTS
(pathfinder init) |        57.5|3.9       |
|A (static delays)       |Pathfinder            |         7.1|-         |
|B (time-varying delays) |NUTS
(prior init)      |       148.2|1.6       |
|B (time-varying delays) |NUTS
(pathfinder init) |       126.3|1.6       |
|B (time-varying delays) |Pathfinder            |        12.9|-         |
...
|A: NUTS (pathfinder init) |    1000|     1.01|                     1|             10|
|B: NUTS (prior init)      |    1000|     1.02|                     1|             10|
|B: NUTS (pathfinder init) |    1000|     1.03|                     2|             10|
...
We do not recommend using pathfinder as the final inference method for these models — it is useful for rapid iteration during model development and as an initialiser for NUTS.
...
|:-----------------------|:----------------------|:----------------|----:|-------------:|
|A (static delays)       |NUTS (prior init)      |refp_mean_int[1] | 3.95|          1.92|
|A (static delays)       |NUTS (prior init)      |refp_sd_int[1]   | 4.16|          0.80|
|A (static delays)       |NUTS (prior init)      |sqrt_phi[1]      | 1.83|          0.27|
|A (static delays)       |NUTS (pathfinder init) |refp_mean_int[1] | 3.97|          2.01|
|A (static delays)       |NUTS (pathfinder init) |refp_sd_int[1]   | 4.16|          0.85|
|A (static delays)       |NUTS (pathfinder init) |sqrt_phi[1]      | 1.83|          0.27|
|A (static delays)       |Pathfinder             |refp_mean_int[1] | 3.87|          0.24|
|A (static delays)       |Pathfinder             |refp_sd_int[1]   | 4.15|          0.04|
|A (static delays)       |Pathfinder             |sqrt_phi[1]      | 1.75|          0.04|
|B (time-varying delays) |NUTS (prior init)      |refp_mean_int[1] | 3.63|          2.20|
|B (time-varying delays) |NUTS (prior init)      |refp_sd_int[1]   | 3.92|          1.13|
|B (time-varying delays) |NUTS (prior init)      |sqrt_phi[1]      | 1.73|          0.26|
|B (time-varying delays) |NUTS (pathfinder init) |refp_mean_int[1] | 3.66|          2.33|
|B (time-varying delays) |NUTS (pathfinder init) |refp_sd_int[1]   | 3.88|          1.21|
|B (time-varying delays) |NUTS (pathfinder init) |sqrt_phi[1]      | 1.74|          0.28|
|B (time-varying delays) |Pathfinder             |refp_mean_int[1] | 3.88|          0.32|
|B (time-varying delays) |Pathfinder             |refp_sd_int[1]   | 4.09|          0.21|
|B (time-varying delays) |Pathfinder             |sqrt_phi[1]      | 1.75|          0.12|



Across all three parameters the point estimates (means) are broadly aligned between NUTS and pathfinder.
The 90% credible intervals from pathfinder are roughly an order of magnitude narrower than the NUTS references — for example, for `refp_mean_int` in spec A the NUTS width is around 1.9 on the log scale versus around 0.24 for pathfinder (about 8× narrower); for spec B the gap is similar (~2.2 versus ~0.3).
...
An alternative workflow is to run NUTS once to obtain a high-quality posterior and then use that posterior (with slightly inflated standard deviations) as the prior for subsequent pathfinder updates, refreshing the NUTS fit periodically (e.g. weekly).
This mirrors the approach used in the [Germany age-stratified vignette](germany-age-stratified-nowcasting.html).

We demonstrate this using spec A.
First, we extract posterior summaries from the spec A NUTS fit and inflate the standard deviations to give the updated model some room to move.


``` r
day1_posterior <- summary(
  fit_a_nuts, type = "fit",
  variables = c("refp_mean_int", "refp_sd_int", "sqrt_phi")
)
day1_posterior[, sd := sd * 1.5]
day1_posterior
#>            variable     mean   median        sd        mad       q5      q20
#>              <char>    <num>    <num>     <num>      <num>    <num>    <num>
#> 1: refp_mean_int[1] 3.953592 3.920542 0.8898323 0.57345463 3.050905 3.477463
#> 2:   refp_sd_int[1] 4.156285 4.152728 0.3745551 0.25346300 3.763707 3.945196
#> 3:      sqrt_phi[1] 1.830416 1.827263 0.1272552 0.08769349 1.700937 1.756875
#>         q80      q95     rhat ess_bulk ess_tail
#>       <num>    <num>    <num>    <num>    <num>
#> 1: 4.464302 4.969530 1.002663 498.3005 616.5455
#> 2: 4.369503 4.560493 1.003834 488.3490 665.0291
#> 3: 1.905845 1.972624 1.003869 836.8029 635.2251
```
...
Rebuild the shared modules for the day 2 data and define a factory that fixes everything except the fit options and priors.
...
We fit day 2 with pathfinder using the day 1 posterior as informative priors (the cheap daily update) and with a full NUTS refit using default priors (the gold-standard reference we are trying to match).


``` r
fit_day2_pf <- fit_day2(
  priors = day1_posterior,
  fit = enw_fit_opts(sampler = enw_pathfinder, pp = TRUE)
)
```
...
update_runtime <- data.table(
  ...
  `Runtime (s)` = c(fit_day2_pf$run_time, fit_day2_nuts$run_time)
)
...
|Method                            | Runtime (s)|
|:---------------------------------|-----------:|
|Pathfinder (informed priors)      |         5.1|
|NUTS (default priors, full refit) |       118.3|



We check whether the cheap pathfinder update tracks the full NUTS refit, both on the nowcast and the underlying parameter posteriors.


``` r
update_fits <- list(
  "Pathfinder (informed priors)" = fit_day2_pf,
  "NUTS (default priors, full refit)" = fit_day2_nuts
)

update_nc <- rbindlist(lapply(names(update_fits), function(nm) {
  s <- summary(update_fits[[nm]], type = "nowcast")
  s[, method := nm]
  s
}))
...
``` r
update_params <- rbindlist(lapply(
  names(update_fits), function(nm) {
    s <- summary(
      update_fits[[nm]], type = "fit",
      variables = c("refp_mean_int", "refp_sd_int", "sqrt_phi")
    )
    s[, method := nm]
    s
