...
(prior init)      |        46.3|5.1       |
|A (static delays)       |NUTS
(pathfinder init) |        43.1|4.4       |
|A (static delays)       |Pathfinder            |         6.7|-         |
|B (time-varying delays) |NUTS
(prior init)      |       100.2|2.3       |
|B (time-varying delays) |NUTS
(pathfinder init) |       103.6|1.7       |
|B (time-varying delays) |Pathfinder            |        12.5|-         |
...
|A: NUTS (pathfinder init) |    1000|     1.02|                     3|             10|
|B: NUTS (prior init)      |    1000|     1.01|                     4|              9|
|B: NUTS (pathfinder init) |    1000|     1.01|                     4|              9|
...
|:-----------------------|:----------------------|:----------------|----:|-------------:|
|A (static delays)       |NUTS (prior init)      |refp_mean_int[1] | 3.99|          1.91|
|A (static delays)       |NUTS (prior init)      |refp_sd_int[1]   | 4.18|          0.83|
|A (static delays)       |NUTS (prior init)      |sqrt_phi[1]      | 1.83|          0.26|
|A (static delays)       |NUTS (pathfinder init) |refp_mean_int[1] | 3.94|          1.93|
|A (static delays)       |NUTS (pathfinder init) |refp_sd_int[1]   | 4.16|          0.82|
|A (static delays)       |NUTS (pathfinder init) |sqrt_phi[1]      | 1.83|          0.27|
|A (static delays)       |Pathfinder             |refp_mean_int[1] | 5.10|          0.16|
|A (static delays)       |Pathfinder             |refp_sd_int[1]   | 4.49|          0.23|
|A (static delays)       |Pathfinder             |sqrt_phi[1]      | 1.84|          0.17|
|B (time-varying delays) |NUTS (prior init)      |refp_mean_int[1] | 3.66|          2.08|
|B (time-varying delays) |NUTS (prior init)      |refp_sd_int[1]   | 3.89|          1.21|
|B (time-varying delays) |NUTS (prior init)      |sqrt_phi[1]      | 1.74|          0.27|
|B (time-varying delays) |NUTS (pathfinder init) |refp_mean_int[1] | 3.67|          2.34|
|B (time-varying delays) |NUTS (pathfinder init) |refp_sd_int[1]   | 3.92|          1.21|
|B (time-varying delays) |NUTS (pathfinder init) |sqrt_phi[1]      | 1.74|          0.26|
|B (time-varying delays) |Pathfinder             |refp_mean_int[1] | 4.18|          0.05|
|B (time-varying delays) |Pathfinder             |refp_sd_int[1]   | 4.03|          0.11|
|B (time-varying delays) |Pathfinder             |sqrt_phi[1]      | 1.69|          0.10|
...
**Note:** A more refined way to handle this would be an `update()` method for `enw_preprocess_data` objects, which is tracked as a future improvement.
...
#>         variable        mean         sd
#>           <char>       <num>      <num>
#> 1:    expr_r_int -0.01983269 0.11839419
#> 2:  expr_beta_sd  0.23877226 0.09575963
#> 3: refp_mean_int  3.98715066 0.59298997
#> 4:   refp_sd_int  4.17938269 0.24989501
#> 5:   rep_beta_sd  0.42932293 0.13743612
#> 6:      sqrt_phi  1.82694723 0.07933930
```
...
# Capture shared model spec using formulas
fit_day2 <- function(data, fit_opts, priors = NULL) {
  args <- list(
    data = data,
    reference = enw_reference(
      parametric = ~1, distribution = "lognormal", data = data
    ),
    expectation = enw_expectation(
      ~ 1 + rw(week) + (1 | day_of_week), data = data
    ),
    report = enw_report(~ (1 | day_of_week), data = data),
    obs = enw_obs(family = "negbin1d", data = data),
    model = model,
    fit = fit_opts
  )
  if (!is.null(priors)) {
...
``` r
# Pathfinder with informed priors
fit_day2_pf_informed <- fit_day2(
  pobs_day2, priors = day1_posterior,
  fit_opts = enw_fit_opts(sampler = enw_pathfinder, pp = TRUE)
)

# Pathfinder with default priors
fit_day2_pf_default <- fit_day2(
  pobs_day2,
  fit_opts = enw_fit_opts(sampler = enw_pathfinder, pp = TRUE)
)

# NUTS with default priors (full refit)
fit_day2_nuts <- fit_day2(
  pobs_day2, fit_opts = nuts_prior_opts
)
```
...
|Method                            | Runtime (s)|
|:---------------------------------|-----------:|
|Pathfinder (informed priors)      |         2.0|
|Pathfinder (default priors)       |         5.4|
|NUTS (default priors, full refit) |        46.6|
...
``` r
update_fits <- list(
  "Pathfinder (informed)" = fit_day2_pf_informed,
  "Pathfinder (default)" = fit_day2_pf_default,
