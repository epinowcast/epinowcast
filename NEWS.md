...
- Added an optional susceptible-depletion (population) adjustment to the renewal expectation model via the new `population`, `population_period`, `population_floor`, `population_uncertain`, and `population_cv` arguments to `enw_expectation()`.
  ...
  When fitted, each group's initial susceptible population is estimated independently from a shared prior.
  The adjusted recursion is applied to the whole modelled series so the susceptible pool stays internally consistent; `population_period` is retained for forward compatibility but does not currently change the recursion.
