...
- Added a new action to check that the `cmdstan` model can be compiled and has the correct syntax. This runs on PRs whenever stan code is changed, when code is merged onto `main` with altered stan code, and on a weekly schedule against the latest `main` branch. See # by @seabbs.
