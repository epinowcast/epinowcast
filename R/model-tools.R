...
enw_formula_as_data_list <- function(formula, prefix,
  ...
  data <- list()
  data[c("fintercept", "fdesign", "fnrow", "findex", "fnindex", "fncol", "rdesign", "rncol")] <- 0
  data[c("fdesign", "fnrow", "findex", "fnindex", "fncol", "rdesign", "rncol")] <- list(numeric())
  if (!missing(formula)) {
    if (!inherits(formula, "enw_formula")) {
      stop("formula must be an object of class enw_formula as produced using enw_formula")
    }
    fintercept <-  as.numeric(any(grepl("(Intercept)", colnames(formula$fixed$design), fixed = TRUE)))
    ...
    data$fncol <- ncol(formula$fixed$design) - min(as.numeric(drop_intercept), fintercept)
