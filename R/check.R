...
check_modules_compatible <- function(modules) {
  if (modules[[4]]$data$model_miss && !modules[[6]]$data$likelihood_aggregation) {
    warning(paste0("Incompatible model specification: A missingness model has ",
                   ...
                   "argument in `enw_fit_opts` to 'groups'."), immediate. = T)
